...
void caracterDibujar(const tCaracter *carac, tCursorTexto *cursor, int color)   //Dibuja un caracter a partir de darle un tCaracter como parámetro
{
    ...
    for (i = 0; i < carac->espaciado; i++)
    {
        for (j = 0; j < ALTO_ESTANDAR_LETRA; j++)
        {
            ...
            if (*p == 'X')
                gbt_dibujar_pixel(cursor->posX + i, cursor->posY + j, color);
        }
    }

    cursor->posX += carac->espaciado + ESPACIADO_ENTRE_LETRAS;    //Avanza el cursor en cierta cantidad para dar espacio a la siguiente letra (en caso de haber)
}
...
int obtenerIndice(char caracter)        //Si le das un char, devuelve el indice correspondiente del vector fuente
{
    ...
    if (caracter >= 'a' && caracter <= 'z')
        return caracter - 71;
    if (caracter >= '0' && caracter <= '9')     //Si si pasa al numero como char
        return caracter - 21;
    if (caracter >= 0 && caracter <= 9)         //Si se pasa al numero como int
        return caracter + 27;
...
void caracterDibujarChar(char carac, tCursorTexto *cursor, int color, int tipoFuente)     //Dibuja un caracter a partir de darle un char como parámetro
{
    ...
    cursor->posX += fuente1[obtenerIndice(carac)].espaciado + ESPACIADO_ENTRE_LETRAS;   //Avanza el cursor en cierta cantidad para dar espacio a la siguiente letra (en caso de haber)
}

void caracterNumeroDibujar(int numero, tCursorTexto *cursor, int color)
{
    int i, j;
    const tCaracter *caracter = &fuente1[obtenerIndice(numero)];
    const char *p;
...
void escribirTexto(const char str[], tCursorTexto *cursor, int color, int centrado, int tipoFuente)
{
    ...
    while(*p != '\0')
    {
        caracterDibujarChar(*p, &cursorAux, color, tipoFuente);
        p++;
    }
}
...
void escribirNumero(int numero, tCursorTexto *cursor, int color, int centrado, int tipofuente)
{
    ...
    escribirTexto(string, cursor, color, centrado, tipofuente);
}
