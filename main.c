...
int main(int argc, char *argv[])
{
    ...
    tGBT_Temporizador *tempFijacion = gbt_temporizador_crear(velActual/2);    //Inicialización del temporizador para inactivar tetromino
    if (!tempFijacion)
        return ERROR_CREAR_TEMPORIZADOR;
    ...
    while(estadoDeJuego)
    {
        ...
        case JUGANDO:
            if (partidaNueva)
            {
                ...
                tempCaida = gbt_temporizador_crear(velActual);    //Inicialización del temporizador para la caida
                gbt_temporizador_destruir(tempFijacion);
                tempFijacion = gbt_temporizador_crear(velActual/2);    //Inicialización del temporizador para la fijacion
