...
int main(int argc, char *argv[])
{
    ...
    int vectorInfoJuego[CANT_TETROMINOS_DELUXE + DATOS_DE_JUEGO];
    ...
    while(estadoDeJuego)
    {
        switch (estadoDeJuego)
        {
            ...
            case MENU_PRINCIPAL_CLASSIC:
                cursorBoton = 0;
                vectorInfoJuego[MODO_DE_JUEGO] = MODO_CLASSIC;
            ...
            case MENU_PRINCIPAL_DELUXE:
                cursorBoton = 0;
                vectorInfoJuego[MODO_DE_JUEGO] = MODO_DELUXE;
            ...
            case JUGANDO:
                if (partidaNueva)
                {
                    ...
                    vectorInfoJuego[LINEAS] = 0;
                    vectorInfoJuego[SCORE] = 0;
                    vectorInfoJuego[TOP_SCORE] = 0;
                    vectorInfoJuego[NIVEL] = 1;
                    grillaDestruir(&grillaDeFondo);
                    grillaCrear(&grillaDeFondo, resolAncho, resolAlto, vectorInfoJuego[MODO_DE_JUEGO] ? anchoGrilla : ANCHO_GRILLA_DEFAULT);
                    partidaNueva = false;
                }
                ...
                while (estadoDeJuego == JUGANDO)
                    estadoDeJuego = interfazJuego(vectorInfoJuego, tetroActivos, &grillaDeFondo, &tempCaida, &tempFijacion, &velActual, &modoVelocidad);
                break;
            ...
            case GAME_OVER:
                while (estadoDeJuego == GAME_OVER)
                    estadoDeJuego = gameOver(resolAlto, resolAncho);
                break;
        }
    }
